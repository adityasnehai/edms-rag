...
const TYPE_STYLES = {
  adrs: { badge: "bg-violet-50 text-violet-700 border-violet-200", bar: "bg-violet-500" },
  rfcs: { badge: "bg-sky-50 text-sky-700 border-sky-200", bar: "bg-sky-500" },
  meeting_notes: { badge: "bg-emerald-50 text-emerald-700 border-emerald-200", bar: "bg-emerald-500" },
  postmortems: { badge: "bg-rose-50 text-rose-700 border-rose-200", bar: "bg-rose-500" },
  tickets: { badge: "bg-amber-50 text-amber-700 border-amber-200", bar: "bg-amber-500" },
  images: { badge: "bg-indigo-50 text-indigo-700 border-indigo-200", bar: "bg-indigo-500" },
};

const DEFAULT_STYLE = { badge: "bg-accent text-accent-foreground border-primary/20", bar: "bg-primary" };
    ...
    return (
      <article className="relative flex h-full min-w-[18rem] max-w-[18rem] snap-start flex-col overflow-hidden rounded-2xl border border-border bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-primary/20 hover:shadow-md sm:min-w-[21rem] sm:max-w-[21rem]">
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <div className="mt-3 flex-1 rounded-xl border border-border bg-slate-50 p-3">
            {expanded ? (
              <div className="max-h-80 overflow-y-auto pr-1">
                <FormattedContent
                  text={item.text || "No extracted text available for this record."}
                  paragraphClassName="text-sm leading-7 text-slate-600"
                  listClassName="space-y-1.5 pl-4 text-sm leading-7 text-slate-600"
                  headingClassName="text-slate-800 font-semibold"
                  strongClassName="text-slate-800"
                />
              </div>
            ) : (
          ...
          <button
            ...
            className="mt-3 inline-flex items-center justify-center rounded-xl border border-primary/20 bg-accent px-3 py-2 text-xs font-semibold text-primary transition hover:bg-primary hover:text-white"
          >
    ...
    <article className="relative overflow-hidden rounded-2xl border border-border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className={`${compact ? "p-4 pl-5" : "p-5 pl-6"} space-y-4`}>
            ...
            {expanded ? (
              <div className="max-h-[22rem] overflow-y-auto pr-1">
                <FormattedContent
                  ...
                  paragraphClassName={compact ? "text-sm leading-7 text-slate-600" : "text-[15px] leading-8 text-slate-600"}
                  listClassName={compact ? "space-y-1.5 pl-4 text-sm leading-7 text-slate-600" : "space-y-2 pl-5 text-[15px] leading-8 text-slate-600"}
                  headingClassName="text-slate-800 font-semibold"
                  strongClassName="text-slate-800"
                />
              </div>
        ...
        <button
          type="button"
          onClick={() => setExpanded((value) => !value)}
          className="inline-flex items-center justify-center rounded-xl border border-primary/20 bg-accent px-3 py-2 text-xs font-semibold text-primary transition hover:bg-primary hover:text-white"
        >
