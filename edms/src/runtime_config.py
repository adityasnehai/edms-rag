...
RERANKER_WARMUP_ON_STARTUP = os.getenv(
    "RERANKER_WARMUP_ON_STARTUP",
    "1" if PRODUCTION_MODE else "0",
).strip().lower() in {"1", "true", "yes"}
RERANKER_CACHE_DIR = os.getenv(
    "RERANKER_CACHE_DIR",
    "/tmp/edms-reranker-cache",
).strip()
RERANKER_SLOW_THRESHOLD_SECONDS = max(
    0.1,
    _float_env("RERANKER_SLOW_THRESHOLD_SECONDS", 1.5),
)
RERANKER_DEGRADE_TTL_SECONDS = max(
    30,
    _int_env("RERANKER_DEGRADE_TTL_SECONDS", 300),
)
RERANKER_MAX_DOC_CHARS = max(
    500,
    _int_env("RERANKER_MAX_DOC_CHARS", 4000),
)
RERANKER_BATCH_SIZE = max(1, _int_env("RERANKER_BATCH_SIZE", 16))
RERANKER_VERSION = os.getenv(
    "RERANKER_VERSION",
    f"local:{RERANKER_MODEL}",
)

HF_HOME = os.getenv("HF_HOME", RERANKER_CACHE_DIR).strip()
TRANSFORMERS_CACHE = os.getenv("TRANSFORMERS_CACHE", f"{HF_HOME}/transformers").strip()
SENTENCE_TRANSFORMERS_HOME = os.getenv(
    "SENTENCE_TRANSFORMERS_HOME",
    f"{HF_HOME}/sentence-transformers",
).strip()
...
INGESTION_BATCH_JOB_LIMIT = max(1, _int_env("INGESTION_BATCH_JOB_LIMIT", 20))
...
MAX_REQUEST_BODY_SIZE_BYTES = max(
    1024 * 1024,
    _int_env("MAX_REQUEST_BODY_SIZE_BYTES", 1024 * 1024 * 10),
)
...
LOGIN_FAILURE_LOCKOUT_THRESHOLD = max(3, _int_env("LOGIN_FAILURE_LOCKOUT_THRESHOLD", 5))
LOGIN_FAILURE_LOCKOUT_SECONDS = max(60, _int_env("LOGIN_FAILURE_LOCKOUT_SECONDS", 300))
...
WORKSPACE_INSIGHTS_CACHE_TTL_SECONDS = max(
    30,
    _int_env("WORKSPACE_INSIGHTS_CACHE_TTL_SECONDS", 60 * 10),
)
