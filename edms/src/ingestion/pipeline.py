...
from src.runtime_config import PRODUCTION_MODE
...
def _process_job(job_id: str) -> None:
    ...
    try:
        ...
        if job.get("data_type") == "images" and image_stats.get("failed", 0) > 0:
        ...
        if meta.get("status") == "error":
            ...
            if INGESTION_JOBS_TOTAL:
                INGESTION_JOBS_TOTAL.labels(status="failed", trigger_source=job.get("trigger_source") or "unknown", data_type=job.get("data_type") or "unknown").inc()
            return
        ...
        if INGESTION_JOBS_TOTAL:
            INGESTION_JOBS_TOTAL.labels(status="completed", trigger_source=job.get("trigger_source") or "unknown", data_type=job.get("data_type") or "unknown").inc()
        log_event(20, "ingestion_completed", org_slug=job["org_slug"], job_id=job_id)
        ...
        if INGESTION_JOBS_TOTAL:
            INGESTION_JOBS_TOTAL.labels(status="failed", trigger_source=job.get("trigger_source") or "unknown", data_type=job.get("data_type") or "unknown").inc()
        log_event(40, "ingestion_failed", org_slug=job["org_slug"], job_id=job_id, error_type=exc.__class__.__name__)
